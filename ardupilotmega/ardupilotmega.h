...
typedef enum MAV_CMD
{
   ...
   MAV_CMD_NAV_LOITER_UNLIM=17, /* Loiter around this waypoint an unlimited amount of time |1: Empty| 2: Empty| 3: e, else counter-clockwise| 4: Desired yaw angle.| 5: Latitude| 6: Longitude| 7: Altitude|  */
   ...
   MAV_CMD_DO_DIGICAM_CONFIGURE=202, /* Mission command to configure an on-board camera controller system. |1: Modes: P, TV, AV, M, Etc| 2: Shutter speed: Divisor number for one second| 3: Aperture: F stop number| 4: ISO number e.g. 80, 100, 200, Etc| 5: Exposure type enumerator| 6: Command Identity| 7: Main engine cut-off time before camera trigger in seconds/10 (0 means no cut-off)|  */
   MAV_CMD_DO_DIGICAM_CONTROL=203, /* Mission command to control an on-board camera controller system. |1: Session control e.g. show/hide lens| 2: Zoom's absolute position| 3: Zooming step value to offset zoom from the current position| 4: Focus Locking, Unlocking or Re-locking| 5: Shooting Command| 6: Command Identity| 7: Test shot identifier. If set to 1, image will only be captured, but not counted towards internal frame count.|  */
   MAV_CMD_DO_MOUNT_CONFIGURE=204, /* Mission command to configure a camera or antenna mount |1: Mount operation mode (see MAV_MOUNT_MODE enum)| 2: stabilize roll? (1 = yes, 0 = no)| 3: stabilize pitch? (1 = yes, 0 = no)| 4: stabilize yaw? (1 = yes, 0 = no)| 5: Empty| 6: Empty| 7: Empty|  */
   MAV_CMD_DO_MOUNT_CONTROL=205, /* Mission command to control a camera or antenna mount |1: pitch (WIP: DEPRECATED: or lat in degrees) depending on mount mode.| 2: roll (WIP: DEPRECATED: or lon in degrees) depending on mount mode.| 3: yaw (WIP: DEPRECATED: or alt in meters) depending on mount mode.| 4: WIP: alt in meters depending on mount mode.| 5: WIP: latitude in degrees * 1E7, set if appropriate mount mode.| 6: WIP: longitude in degrees * 1E7, set if appropriate mount mode.| 7: MAV_MOUNT_MODE enum value|  */
   MAV_CMD_DO_SET_CAM_TRIGG_DIST=206, /* Mission command to set camera trigger distance for this flight. The camera is triggered each time this distance is exceeded. This command can also be used to set the shutter integration time for the camera. |1: g.| 2: Camera shutter integration time (milliseconds). -1 or 0 to ignore| 3: Trigger camera once immediately. (0 = no trigger, 1 = trigger)| 4: Empty| 5: Empty| 6: Empty| 7: Empty|  */
   ...
   MAV_CMD_LOGGING_START=2510, /* Request to start streaming logging data over MAVLink (see also LOGGING_DATA message) |1: Format: 0: ULog| 2: Reserved (set to 0)| 3: erved (set to 0)| 4: Reserved (set to 0)| 5: Reserved (set to 0)| 6: Reserved (set to 0)| 7: Reserved (set to 0)|  */
   ...
   MAV_CMD_WAYPOINT_USER_3=31002, /* User defined waypoint item. Ground Station will show the Vehicle as flying through this item. |1: User defined| 2: er defined| 3: User defined| 4: User defined| 5: Latitude unscaled| 6: Longitude unscaled| 7: Altitude (MSL), in meters|  */
   ...
   MAV_CMD_SPATIAL_USER_5=31009, /* User defined spatial item. Ground Station will not show the Vehicle as flying through this item. Example: ROI item. |1: User defined| 2: User defined| 3: User defined| 4: User defined| 5: caled| 6: Longitude unscaled| 7: Altitude (MSL), in meters|  */
   ...
   MAV_CMD_USER_4=31013, /* User defined command. Ground Station will not show the Vehicle as flying through this item. Example: MAV_CMD_DO_SET_PARAMETER item. |1: defined| 2: User defined| 3: User defined| 4: User defined| 5: User defined| 6: User defined| 7: User defined|  */
   ...
   MAV_CMD_FIXED_MAG_CAL_FIELD=42005, /* Magnetometer calibration based on fixed expected field values in milliGauss. |1: FieldX.| 2: FieldY.| 3: FieldZ.| 4: Empty.| 5: Empty.| 6: Empty.| 7: Empty.|  */
   MAV_CMD_DO_START_MAG_CAL=42424, /* Initiate a magnetometer calibration. |1: uint8_t bitmask of magnetometers (0 means all).| 2: Automatically retry on failure (0=no retry, 1=retry).| 3: quire input, 1=autosave).| 4: Delay (seconds).| 5: Autoreboot (0=user reboot, 1=autoreboot).| 6: Empty.| 7: Empty.|  */
   MAV_CMD_DO_ACCEPT_MAG_CAL=42425, /* Initiate a magnetometer calibration. |1: uint8_t bitmask of magnetometers (0 means all).| 2: Empty.| 3: Empty.| 4: Empty.| 5: Empty.| 6: Empty.| 7: Empty.|  */
   MAV_CMD_DO_CANCEL_MAG_CAL=42426, /* Cancel a running magnetometer calibration. |1: uint8_t bitmask of magnetometers (0 means all).| 2: Empty.| 3: Empty.| 4: Empty.| 5: Empty.| 6: Empty.| 7: Empty.|  */
   MAV_CMD_SET_FACTORY_TEST_MODE=42427, /* Command autopilot to get into factory test/diagnostic mode. |1: 0 means get out of test mode, 1 means get into test mode.| 2: Empty.| 3: Empty.| 4: Empty.| 5: Empty.| 6: Empty.| 7: Empty.|  */
   ...
   MAV_CMD_GIMBAL_AXIS_CALIBRATION_STATUS=42502, /* Reports progress and success or failure of gimbal axis calibration procedure. |1: Gimbal axis we're reporting calibration progress for.| 2: Current calibration progress for this axis, 0x64=100%.| 3: Status of the calibration.| 4: Empty.| 5: Empty.| 6: Empty.| 7: Empty.|  */
   ...
   MAV_CMD_DO_WINCH=42600, /* Command to operate winch. |1: Winch number (0 for the default winch, otherwise a number from 1 to max number of winches on the vehicle).| 2: Action (0=relax, 1=relative length control, 2=rate control. See WINCH_ACTIONS enum.).| 3: Release length (cable distance to unwind in meters, negative numbers to wind in cable).| 4: Release rate (meters/second).| 5: Empty.| 6: Empty.| 7: Empty.|  */
...
#include "./mavlink_msg_depth_hold.h"
#include "./mavlink_msg_attitude_hold.h"
#include "./mavlink_msg_flip_trick.h"
#include "./mavlink_msg_lab_depth_pid.h"
